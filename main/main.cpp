...
void sendCanData(uint32_t id, uint8_t dlc, uint8_t *data) {
    ...
    ESP_LOGI(TAG1, "sendCanData - CAN message with ID 0x%X transmitted", (unsigned int)message.identifier);
...
void receiveMotorState(Actuator& motor) {
    ...
    if (err == ESP_OK) {
        if (message.identifier == 1) { //need to change the 1 to motor_id
            ...
            cout << TAG1 << ": receiveMotorState - Received Motor Position: " << fixed << setprecision(2) << motorState.position << ", Velocity: " << motorState.velocity 
            << ", Torque: " << motorState.torque << endl;
        }
    } else if (err == ESP_ERR_TIMEOUT) {
        cout << TAG1 << ": receiveMotorState - CAN receive timeout" << endl;
    } else {
        cout << TAG1 << ": receiveMotorState - CAN receive error: " << esp_err_to_name(err) << endl;
    }
}
...
extern "C" void app_main() {
    // 1. Initialize CAN
    Actuator::init_can();
    cout << TAG1 << ": CAN initialized" << endl;
    ...
    cout << TAG1 << ": Motor enabled and zero position set" << endl;
    /*
    ...
    cout << TAG1 << ": Sending move command to motor" << endl;
    ...
    */
    ...
    cout << TAG1 << ": Moving motor continuously for 30 seconds" << endl;
